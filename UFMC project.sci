...
M = 2;
nblock= 1;
bits= round(rand(1, data*log(M)/log(2)*nblock));
fs= 20e6;
rs= fs/data;
rb= rs*log(M)/log(2);
tb= 1/rb
...
//changing the negative frequency to the positive frequency
k_data= [[-64 :-1],[0:63]];
...
taps= round(3/4*Nfft);
L= Nfft+taps-1; //filtered output length
nor_cutoff= 10/128;  
n= 0:taps-1;
[h_c,H_k,fre]=wfir('lp',taps,nor_cutoff,'hm',[0,0])
Co=1.5;
[h_c,H_k,fre]=wfir('lp',taps,nor_cutoff,'hm',[0,0])
h_n=h_c.*exp(2i*%pi*(-Co)/Nfft*n)
...
figure
subplot(3,1,1);title('IMPULSE RESPONSE OF FREQUENCY SHIFTED PROTOTYPE FILTER(h_n)');plot(h_n);xlabel('Sample Index (n)');ylabel('Amplitude')
subplot(3,1,2);title('FREQUENCY RESPONSE OF FREQUENCY SHIFTED PROTOTYPE FILTER (H_K)');plot(abs(H_K))
xlabel('Frequency Bin (k)');ylabel('|H(k)|')
subplot(3,1,3);title('PHASE  RESPONSE OF FREQUENCY SHIFTED PROTOTYPE FILTER(H_K)');plot(unwrap(angle(H_K)))
xlabel('Frequency Bin (k)');ylabel('Phase ')
...
figure
x_sum=zeros(1,Nfft+taps-1)     
s_sum=zeros(1,Nfft)
sub_S=zeros(1,Nfft);
X_sum=zeros(1,2*Nfft)
for ii=1:nblock
    for j=1:subband
        S=zeros(1,Nfft)
       ...
       subplot(3,1,1);title('FREQUENCY RESPONSE OF H_Shift');plot(abs((H_Shift)))
       xlabel('Frequency Bin (k)');ylabel('|H(k)|')
       subplot(3,1,2);title('PHASE RESPONSE OF SUBBAND')
       plot(unwrap(angle(H_Shift(1:16))))
       xlabel('Frequency Bin (k)');ylabel('Phase')
       x=convol(s,h_shift)
       x_sum=x_sum+x
...
xsum=ifft([X_sum ])
xsum_fft=X_sum
xsum_fft_down=xsum_fft(1:2:end) //fft down conversion
...
for p=1:subband
    ...
    kp(p) = p_idx;
    Hp(p) = xsum_fft_down(p_idx)/sub_S([k_data_pilot(p)]+1); 
     //pilot estimation
    Xeq_pilot=xsum_fft_down(p_idx)/Hp(p)      
     //equivalsing the pilot
